...
module cut() {
	hull() {
		translate([0,0,10])cylinder(r=9.5,h=50,center=true,$fn=res);
		translate([0,0,-29])cylinder(r=9.5,h=1,$fn=res);
	}
...
difference() {
	hull()union(){
	translate([0,0,38])cylinder(r=14.5,h=10,center=true,$fn=res);	
...
//Render
module top() {
		translate([0,0,-3])cylinder(r=11,h=16,$fn=res);
		translate([0,0,0]){
			thread_out(24,12);
